...
func progpowLight(size uint64, cache []uint32, hash []byte, nonce uint64,
	...
	return progpowHash(hash, nonce, size, blockNumber, cDag, lookup)
}
...
func progpowFull(dataset []uint32, hash []byte, nonce uint64, blockNumber uint64) ([]byte, []byte) {
	...
	return progpowHash(hash, nonce, uint64(len(dataset))*4, blockNumber, cDag, lookup)
}
...
func progpowHash(hash []byte, nonce uint64, size uint64, blockNumber uint64, cDag []uint32,
