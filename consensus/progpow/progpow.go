...
func newlru(what string, maxItems int, new func(epoch uint64) interface{}) *lru {
	...
	cache, _ := simplelru.NewLRU(maxItems, func(key, value interface{}) {
		log.Trace("Evicted progpow "+what, "epoch", key)
	})
...
func (lru *lru) get(epoch uint64) (item, future interface{}) {
	...
	if !ok {
		if lru.future > 0 && lru.future == epoch {
			...
			log.Trace("Requiring new progpow "+lru.what, "epoch", epoch)
			item = lru.new(epoch)
		}
	...
	if epoch < maxCachedEpoch-1 && lru.future < epoch+1 {
		log.Trace("Requiring new future progpow "+lru.what, "epoch", epoch+1)
...
// cache wraps a progpow cache with some metadata to allow easier concurrent use.
type cache struct {
...
// newCache creates a new progpow verification cache and returns it as a plain Go
...
// dataset wraps a progpow dataset with some metadata to allow easier concurrent use.
type dataset struct {
	epoch   uint64    // Epoch for which this cache is relevant
	dump    *os.File  // File descriptor of the memory mapped cache
	mmap    mmap.MMap // Memory map itself to unmap before releasing
...
// newDataset creates a new progpow mining dataset and returns it as a plain Go
// interface to be usable in an LRU cache.
...
func (d *dataset) generate(dir string, limit int, lock bool, test bool) {
	d.once.Do(func() {
		...
		if err == nil {
			logger.Debug("Loaded old progpow dataset from disk")
			return
		}
		logger.Debug("Failed to load old progpow dataset", "err", err)
		...
		if err != nil {
			logger.Error("Failed to generate mapped progpow dataset", "err", err)
...
// MakeCache generates a new progpow cache and optionally stores it to disk.
func MakeCache(block uint64, dir string) {
	c := cache{epoch: block / epochLength}
	c.generate(dir, math.MaxInt32, false, false)
}

// MakeDataset generates a new progpow dataset and optionally stores it to disk.
...
// Progpow is a consensus engine based on proof-of-work implementing the progpow
// algorithm.
type Progpow struct {
	config Config
...
func New(config Config, notify []string, noverify bool) *Progpow {
	...
	if config.CachesInMem <= 0 {
		config.Log.Warn("One progpow cache must always be in memory", "requested", config.CachesInMem)
		config.CachesInMem = 1
	}
	if config.CacheDir != "" && config.CachesOnDisk > 0 {
		config.Log.Info("Disk storage enabled for progpow caches", "dir", config.CacheDir, "count", config.CachesOnDisk)
	}
	if config.DatasetDir != "" && config.DatasetsOnDisk > 0 {
		config.Log.Info("Disk storage enabled for progpow DAGs", "dir", config.DatasetDir, "count", config.DatasetsOnDisk)
	}
	progpow := &Progpow{
		config:   config,
...
// all blocks' seal as valid, though they still have to conform to the Yottaflux
// consensus rules.
...
// still have to conform to the Yottaflux consensus rules.
...
// they still have to conform to the Yottaflux consensus rules.
...
// NewFullFaker creates a progpow consensus engine with a full fake scheme that
...
func (progpow *Progpow) Close() error {
	return progpow.StopRemoteSealer()
}

// StopRemoteSealer stops the remote sealer
func (progpow *Progpow) StopRemoteSealer() error {
...
func (progpow *Progpow) dataset(block uint64, async bool) *dataset {
	// Retrieve the requested progpow dataset
	epoch := block / epochLength
	currentI, futureI := progpow.datasets.get(epoch)
	current := currentI.(*dataset)

	// If async is specified, generate everything in a background thread
	if async && !current.generated() {
		go func() {
			current.generate(progpow.config.DatasetDir, progpow.config.DatasetsOnDisk, progpow.config.DatasetsLockMmap, progpow.config.PowMode == ModeTest)

			if futureI != nil {
				future := futureI.(*dataset)
				future.generate(progpow.config.DatasetDir, progpow.config.DatasetsOnDisk, progpow.config.DatasetsLockMmap, progpow.config.PowMode == ModeTest)
			}
		}()
	} else {
		...
		if futureI != nil {
			future := futureI.(*dataset)
			go future.generate(progpow.config.DatasetDir, progpow.config.DatasetsOnDisk, progpow.config.DatasetsLockMmap, progpow.config.PowMode == ModeTest)
		}
	}
	return current
}
...
func (progpow *Progpow) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	// In order to ensure backward compatibility, we expose progpow RPC APIs
