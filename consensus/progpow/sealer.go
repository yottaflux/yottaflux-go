...
func (progpow *Progpow) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if progpow.config.PowMode == ModeFake || progpow.config.PowMode == ModeFullFake {
		...
		default:
			progpow.config.Log.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", progpow.SealHash(block.Header()))
		}
		return nil
	}
	// If we're running a shared PoW, delegate sealing to it
	if progpow.shared != nil {
		return progpow.shared.Seal(chain, block, results, stop)
	}
	...
	if progpow.remote != nil {
		progpow.remote.workCh <- &sealTask{block: block, results: results}
	}
	var (
		pend   sync.WaitGroup
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int, nonce uint64) {
			defer pend.Done()
			progpow.mine(block, id, nonce, abort, locals)
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				progpow.config.Log.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", progpow.SealHash(block.Header()))
			}
			close(abort)
		case <-progpow.update:
			// Thread count was changed on user request, restart
			close(abort)
			if err := progpow.Seal(chain, block, results, stop); err != nil {
...
func (progpow *Progpow) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	// Extract some data from the header
	var (
		header  = block.Header()
		hash    = progpow.SealHash(header).Bytes()
		target  = new(big.Int).Div(two256, header.Difficulty)
		number  = header.Number.Uint64()
		dataset = progpow.dataset(number, false)
	)
	...
	logger := progpow.config.Log.New("miner", id)
	logger.Trace("Started progpow search for new nonces", "seed", seed)
search:
	for {
		select {
		case <-abort:
			// Mining terminated, update stats and abort
			logger.Trace("ProgPow nonce search aborted", "attempts", nonce-seed)
		...
		default:
			...
			// Compute the PoW value of this nonce using ProgPow
			...
			if powBuffer.SetBytes(result).Cmp(target) <= 0 {
				...
				case found <- block.WithSeal(header):
					logger.Trace("ProgPow nonce found and reported", "attempts", nonce-seed, "nonce", nonce)
				case <-abort:
					logger.Trace("ProgPow nonce found but discarded", "attempts", nonce-seed, "nonce", nonce)
				}
				break search
			}
			nonce++
...
func (s *remoteSealer) makeWork(block *types.Block) {
	hash := s.progpow.SealHash(block.Header())
...
func (s *remoteSealer) submitWork(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
	...
	if block == nil {
		s.progpow.config.Log.Warn("Work submitted but none pending", "sealhash", sealhash, "curnumber", s.currentBlock.NumberU64())
		return false
	}
	...
	start := time.Now()
	if !s.noverify {
		if err := s.progpow.verifySeal(nil, header, true); err != nil {
			s.progpow.config.Log.Warn("Invalid proof-of-work submitted", "sealhash", sealhash, "elapsed", common.PrettyDuration(time.Since(start)), "err", err)
			return false
		}
	}
