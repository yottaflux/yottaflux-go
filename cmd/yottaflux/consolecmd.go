...
var (
	...
	consoleCommand = &cli.Command{
		...
		Description: `
The Yottaflux console is an interactive shell for the JavaScript runtime environment
...
The Yottaflux console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
This command allows to open a console on a running Yottaflux node.`,
	}
...
// localConsole starts a new yottaflux node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		return fmt.Errorf("Failed to attach to the inproc yottaflux: %v", err)
	}
...
// remoteConsole will connect to a remote yottaflux instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote yottaflux: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
...
// ephemeralConsole starts a new yottaflux node, attaches an ephemeral JavaScript
...
yottaflux --exec "%s" console`, b.String())
	return nil
}

// dialRPC returns a RPC client which connects to the given endpoint.
// The check for empty endpoint implements the defaulting logic
// for "yottaflux attach" with no argument.
