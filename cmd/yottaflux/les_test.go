...
func (g *yottafluxrpc) killAndWait() {
	g.geth.Kill()
	g.geth.WaitExit()
}

func (g *yottafluxrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.geth.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *yottafluxrpc) addPeer(peer *yottafluxrpc) {
...
func (g *yottafluxrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func startYottafluxWithIpc(t *testing.T, name string, args ...string) *yottafluxrpc {
	ipcName := fmt.Sprintf("yottaflux-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	args = append([]string{"--networkid=42", "--port=0", "--authrpc.port", "0", "--ipcpath", ipcName}, args...)
	t.Logf("Starting %v with rpc: %v", name, args)

	g := &yottafluxrpc{
		name: name,
		geth: runYottaflux(t, args...),
	}
	ipcpath := ipcEndpoint(ipcName, g.geth.Datadir)
	// We can't know exactly how long yottaflux will take to start, so we try 10
...
func initYottaflux(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing yottaflux: %v ", args)
	g := runYottaflux(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
}

func startLightServer(t *testing.T) *yottafluxrpc {
	datadir := initYottaflux(t)
	t.Logf("Importing keys to yottaflux")
	runYottaflux(t, "account", "import", "--datadir", datadir, "--password", "./testdata/password.txt", "--lightkdf", "./testdata/key.prv").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startYottafluxWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *yottafluxrpc {
	datadir := initYottaflux(t)
	return startYottafluxWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
