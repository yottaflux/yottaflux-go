...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "yottaflux.ipc"
	return cfg
}

// makeConfigNode loads yottaflux configuration and creates a blank node instance.
...
// makeFullNode loads yottaflux configuration and creates the Yottaflux backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `yottaflux db freezer-migrate`.")
		}
	}
