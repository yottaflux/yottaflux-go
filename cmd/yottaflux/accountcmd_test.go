...
func TestAccountListEmpty(t *testing.T) {
	geth := runYottaflux(t, "account", "list")
	geth.ExpectExit()
}
...
`
	...
	{
		geth := runYottaflux(t, "account", "list", "--datadir", datadir)
		geth.Expect(want)
		geth.ExpectExit()
	}
	{
		geth := runYottaflux(t, "--datadir", datadir, "account", "list")
		geth.Expect(want)
		geth.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	geth := runYottaflux(t, "account", "new", "--lightkdf")
...
func TestAccountHelp(t *testing.T) {
	geth := runYottaflux(t, "account", "-h")
	geth.WaitExit()
	if have, want := geth.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	geth = runYottaflux(t, "account", "import", "-h")
	geth.WaitExit()
	if have, want := geth.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	geth := runYottaflux(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer geth.ExpectExit()
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	geth := runYottaflux(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	geth := runYottaflux(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer geth.ExpectExit()
	geth.Expect(`
!! Unsupported terminal, password will be echoed.
...
func TestUnlockFlag(t *testing.T) {
	geth := runMinimalYottaflux(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
